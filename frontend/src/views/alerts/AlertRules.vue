<script setup>
// 告警规则 tab：规则表格 + 新增/编辑/删除，labels_filter 用 key=value 动态行编辑
import { onMounted, reactive, ref } from 'vue'
import { ElMessage, ElMessageBox } from 'element-plus'
import {
  listAlertRules, createAlertRule, updateAlertRule, deleteAlertRule, listDevices,
} from '../../api'
import {
...
function openEdit(row) {
  ...
  Object.assign(form, {
    ...
    escalate_minutes: row.escalate_minutes ?? 0,
...
// 组装提交数据：labelRows -> labels_filter 对象
function buildPayload() {
  ...
  return {
    ...
    escalate_minutes: form.escalate_minutes,
...
async function submit() {
  ...
  try {
    if (dialog.isEdit) {
      await updateAlertRule(dialog.id, buildPayload())
      ElMessage.success('规则已更新')
    } else {
      await createAlertRule(buildPayload())
...
async function toggleEnabled(row) {
  const { id, builtin, created_at, ...rest } = row
  await updateAlertRule(id, { ...rest, enabled: row.enabled })
  ...
  <div>
    ...
    >
      <el-form ref="formRef" :model="form" :rules="rules" label-width="100px">
        ...
        <el-form-item label="升级(分钟)">
          <el-input-number v-model="form.escalate_minutes" :min="0" :max="1440" style="width: 140px" />
          <span class="hint">0 表示不升级；告警超过 N 分钟未确认则等级升一级并重发通知</span>
        </el-form-item>
